    ...
    public class AltLandingController : Controller
    {
        ...
        public ActionResult GetUserInfo()
        {
            ...
            if (userinfo_but == "Get UserInfo" || userinfo2_but == "Get UserInfo")
            {
                ...
                {
                    var client = new RestClient(MvcApplication.apiUrl + "/oauth2/v1/userinfo");
            ...
            if (introspection_but == "Token Introspection" || introspection2_but == "Token Introspection")
            {
                ...
                var client = new RestClient(MvcApplication.apiUrl + "/oauth2/v1/introspect");
            ...
            if (revoke_but == "Token Revoke" || revoke2_but == "Token Revoke")
            {
                ...
                var client = new RestClient(MvcApplication.apiUrl + "/oauth2/v1/revoke");
            ...
            if (oidc_but == "Initiate Auth OIDC")
            {
                ...
                string oauthUrl = appSettings["oidc.authServer"] + "/authorize?response_type=code&response_mode=query&client_id=" + appSettings["oidc.spintweb.clientId"] + "&scope=openid profile email address phone groups offline_access&state=" + stateCode + "&redirect_uri=" + appSettings["oidc.spintweb.RedirectUri"];
            ...
            if (oidc_but == "Initiate Implicit OIDC")
            {
                ...
                Random random = new Random();
                string stateCode = random.Next(99999, 1000000).ToString();
                string oauthUrl = appSettings["oidc.authServer"] + "/authorize?response_type=id_token&response_mode=form_post&client_id=" + appSettings["oidc.spintweb.clientId"] + "&scope=openid profile email address phone groups&state=" + stateCode + "&nonce=CWBb0zHdZ92WqBLkyIuExu&redirect_uri=" + appSettings["oidc.spintweb.redirectUri"];
                return Redirect(oauthUrl);
