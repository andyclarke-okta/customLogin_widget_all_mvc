...
namespace OktaJS_SDK.Services
{
    ...
    public class OktaUserMgmt
    {
        ...
        public bool ActivateUser(User oktaUser, out Uri rspUri, bool sendEmail = false)
        {
            rspUri = null;
            bool returnCode = false;
            try
            {
                rspUri = _usersClient.Activate(oktaUser, sendEmail);
                _logger.Info(string.Format("Activated user in Okta. Okta Name: {0}.", oktaUser));
                returnCode = true;
            }
            catch (Exception ex)
            {
        ...
        public CustomUser AddCustomUser(CustomUser newCustomUser)
        {
            ...
            try
            {
                ...
                // rspOktaUser = _usersClient.Add(newCustomUser, false) as CustomUser;
